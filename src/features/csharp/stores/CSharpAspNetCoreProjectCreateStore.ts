import { CSharpAspNetCoreProject } from '@/features/csharp/projects/CSharpAspNetCoreProject';
import { PackageManager } from '@/features/csharp/projects/PackageManager';
import FileSaver from 'file-saver';
import JSZip from 'jszip';
import { action, computed, makeObservable, observable } from 'mobx';

export class CSharpAspNetCoreProjectCreateStore {
	@observable projectName = 'WebApp';
	...
	@action.bound async submit(): Promise<void> {
		...
		const project = new CSharpAspNetCoreProject(
			{ tab: '\t', newLine: '\n' },
			{
